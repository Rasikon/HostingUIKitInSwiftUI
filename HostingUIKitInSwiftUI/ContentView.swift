...
struct ContentView: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            ...
            Button("Проверь меня", action: {showAlert = true} ).alert(isPresented: $showAlert, content: {
                .init(title: Text("Your score!"), message: Text("\(computeScore())"), dismissButton: .default(Text("ОК")))
            })
